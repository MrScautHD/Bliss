...
public class ImmediateRenderer : Disposable {
    ...
    public void
        SetDepthStencilState(DepthStencilStateDescription? depthStencilState) {
    ...
    public void DrawBoundingBox(CommandList commandList, OutputDescription output, Transform transform, BoundingBox box, Color? color = null, bool stipple = true, bool depthTestEnabled = true) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            corners[i] = Vector3.Transform(corners[i], transform.GetTransform());
        }
        ...
        for (int i = 0; i < 8; i++) {
            for (int bit = 0; bit < 3; bit++) {
                if ((i & (1 << bit)) == 0) {
                    int neighbor = i | (1 << bit);
                    DrawLine(commandList, output, corners[i], corners[neighbor], color, stipple, depthTestEnabled);
                }
            }
        }
    }
    ...
    /// <summary>
    /// Gets the per-vertex line data for a line segment.
    /// </summary>
    /// <param name="startPos">Start position of a line</param>
    /// <param name="endPos">End position of a line</param>
    /// <param name="stipple">Enable stipple?</param>
    /// <returns>A <see cref="Vector4"/> which has the line data.</returns>
    Vector4 GetLineData(Vector3 startPos, Vector3 endPos, bool stipple) {
        float lineLength = Vector3.Distance(startPos, endPos);

        float stippleWorldSize = 0.2f;
        float exactStipples = lineLength / stippleWorldSize;

        float adjustedStipples = (float) Math.Floor(exactStipples);
        if (adjustedStipples % 2 == 0) adjustedStipples += 1;

        return new Vector4(lineLength, adjustedStipples, lineLength, stipple ? 1.0F : 0.0F);
    }
    ...
    /// <param name="stipple">Whether to apply a stipple effect to the line. Defaults to true.</param>
    public void DrawLine(CommandList commandList, OutputDescription output, Vector3 startPos, Vector3 endPos, Color? color = null, bool stipple = true, bool depthTestEnabled = true) {
        Color finalColor = color ?? Color.White;
        
        Vector4 lineData = GetLineData(startPos, endPos, stipple);

        // Add start vertex.
        this._tempVertices.Add(new ImmediateVertex3D {
            Position = startPos,
            Color = finalColor.ToRgbaFloatVec4(),
            LineData = lineData with { X = 0 },
        });
        ...
        this.DrawVertices(commandList, output, new Transform(), this._tempVertices, this._tempIndices, PrimitiveTopology.LineList, !depthTestEnabled);
    }
    
    /// <summary>
    /// Draws a list of points as a spline in 3D space, connecting them with lines.
    /// </summary>
    ...
    public void DrawPointsAsSpline(CommandList commandList, OutputDescription output, Vector3[] points, Color? color = null, bool stipple = true, int resolution = 20, bool depthTestEnabled = true) {
        Color finalColor = color ?? Color.White;
        ...
        Vector3[] splinePoints = GenerateSplinePoints(points, resolution);

        if (splinePoints.Length < 2)
            return;

        // Calculate total spline length and distances
        float totalSplineLength = CalculateTotalSplineLength(splinePoints);
        float[] cumulativeDistances = CalculateCumulativeDistances(splinePoints);
        ...
        // Add vertices for each spline point
        for (int i = 0; i < splinePoints.Length; i++) {
            Vector4 lineData = baseLineData with { X = cumulativeDistances[i] };
        ...
        for (int i = 0; i < splinePoints.Length - 1; i++) {
            this._tempIndices.Add((uint) i);
            this._tempIndices.Add((uint) i + 1);
        }
    ...
    // </summary>
    private Vector3[] GenerateSplinePoints(Vector3[] controlPoints, int resolution) {
    ...
    /// <returns></returns>
    private float CalculateTotalSplineLength(Vector3[] splinePoints) {
    ...
    /// <param name="splinePoints"></param>
    /// <returns></returns>
    private float[] CalculateCumulativeDistances(Vector3[] splinePoints) {
    ...
    public void DrawVertices(CommandList commandList, OutputDescription output, Transform transform, List<ImmediateVertex3D> vertices, List<uint>? indices = null, PrimitiveTopology topology = PrimitiveTopology.TriangleList, bool noDepthTest = true) {
        Cam3D? cam3D = Cam3D.ActiveCamera;
