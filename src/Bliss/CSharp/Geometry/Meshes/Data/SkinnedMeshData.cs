...
public class SkinnedMeshData : IMeshData<SkinnedVertex3D> {
    ...
    /// </summary>
    public SkinnedVertex3D[] Vertices { get; private set; }
    ...
    public DeviceBuffer CreateVertexBuffer(GraphicsDevice graphicsDevice) {
        uint bufferSize = this.VertexCount * (uint) Marshal.SizeOf<SkinnedVertex3D>();
    ...
    public BoundingBox GenBoundingBox() {
        ...
        foreach (SkinnedVertex3D vertex in this.Vertices) {
    ...
    public void GenTangents() {
        ...
        for (int i = 0; i < this.Indices.Length; i += 3) {
            ...
            SkinnedVertex3D v1 = this.Vertices[i1];
            SkinnedVertex3D v2 = this.Vertices[i2];
            SkinnedVertex3D v3 = this.Vertices[i3];
        ...
        for (int i = 0; i < this.Vertices.Length; i++) {
            SkinnedVertex3D vertex = this.Vertices[i];
